...
import type { ProFormColumnsType, ProFormInstance, ProFormProps, SubmitterProps } from '@ant-design/pro-form';
...
export type OtherFormProps = Omit<ProFormProps, 'action' | 'onFinish' | 'form' | 'layoutType' | 'submitter'>;
/**
 * 表单的 schema 定义
 */
export type FormColumnType<T = any, ValueType = 'text'> = Omit<ProFormColumnsType<T, ValueType>, 'editable' | 'fieldProps' | 'renderFormItem' | 'columns'> & {
    /**
     ...
     */
    renderFormItem?: (item: any, config: any, form: any, innerRef: InnerRef) => any;
    /**
     * 重新定义 columns 类型
     */
    columns?: FormColumnType<T, ValueType>[] | ((values: any) => FormColumnType<T, ValueType>[]);
};
/**
 * 表单的 InnerRef
 */
