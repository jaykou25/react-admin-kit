...
import { clearTreeSelectCache } from '../../index';
import { useState } from 'react';
import { Button } from 'antd';
import { setGlobal } from '../../../utils';

describe('BaseTreeSelect Component', () => {
  ...
  it('renders basic tree select component', async () => {
  ...
  it('queryParams refetch', async () => {
    const mockApi = jest.fn(async (params) => {
      return new Promise((resolve) => {
        setTimeout(() => {
          if (params.type === 1) {
            resolve([{ value: 1, label: 'Option 1' }]);
          } else if (params.type === 5) {
            resolve([{ value: 5, label: 'Option 5' }]);
          } else {
            resolve([
              { value: 1, label: 'Option 1' },
              { value: 2, label: 'Option 2' },
  ...
  it('onLoad call once on cached componnets', async () => {
    ...
    setGlobal('@@treeSelectDataIsStart', { basic: false });
