...
该组件是对 Antd [Upload](https://ant-design.antgroup.com/components/upload-cn) 组件的封装, 通过对 `fileList` 和 `onChange` 属性的接管, 让它表现为一个标准的 value - onChange 受控组件, 使其能够被直接用在 Form.Item 里.
